import copy


def has_not(var):
    if "~" in var:
        return True
    return False


def is_variable(val):
    if val[0] == val[0].lower():
        return True
    return False


class VariableException(Exception):
    def __init__(self, message):
        print message


class KnowledgeBase:
    def __init__(self):
        self.kb = {}

    def substitution(self, variable_substitutions, sentence_list):
        for i in range(len(sentence_list)):
            variables = sentence_list[i][1].split(",")
            for j in range(len(variables)):
                if is_variable(variables[j]) and variables[j] in variable_substitutions.keys():
                    variables[j] = variable_substitutions[variables[j]]
            sentence_list[i] = (sentence_list[i][0], ",".join(variables))
        return sentence_list

    def ask(self, queries):
        done_queries = []
        return self.resolution(queries, done_queries)

    def resolution(self, queries, done_queries):

        query = queries[0]
        if query in done_queries:
            return False

        done_queries.append(query)
        del(queries[0])
        query_predicate = query[0]
        query_variables = query[1].split(",")
        if query_predicate in self.kb.keys():
            sentence_list = copy.deepcopy(self.kb[query_predicate])
            s_predicate = sentence_list[0][0]
            s_variables = sentence_list[0][1].split(",")

            if s_predicate == query_predicate and len(s_variables) == len(query_variables):
                variable_substitutions = {}
                del sentence_list[0]
                for i in range(len(query_variables)):
                    if "_" in s_variables[i]:
                        variable_substitutions[s_variables[i]] = query_variables[i]

                sentence_list = self.substitution(variable_substitutions, sentence_list)
                queries.extend(sentence_list)
                queries = list(set(queries))

                if len(queries) == 0:
                    return True
                else:
                    return self.resolution(queries, done_queries)
            else:
                raise VariableException("No substitution found!!")
        else:
            raise VariableException("No substitution found!!")

    def tell(self, sentence_list):
        head = None
        head_variables = None
        tail = []
        for predicate, variables in sentence_list:
            if has_not(predicate):
                predicate = predicate.replace("~", "")
                tail.append((predicate, variables))
            else:
                if not head:
                    head = predicate
                    head_variables = variables
                else:
                    print "Not in Horn form!! Exiting ...."
                    exit()
        if head in self.kb.keys():
            self.kb[head].extend(tail)
        else:
            tail.insert(0, (head, head_variables))
            self.kb[head] = tail

    def pretty_dump(self, indent=3):
        for key, value in self.kb.items():
            print('\t' * indent + str(key))
            print('\t' * (indent + 1) + str(value))


class Parser:
    def __init__(self, file_name):
        self.file_name = file_name

    def read_input(self):
        sentences = []
        queries = []
        with open(self.file_name, "r") as f:
            inp = f.read().strip().split("\n")
            num_queries = int(inp[0])
            for i in range(1, num_queries + 1):
                queries.append(inp[i])

            for i in range(num_queries+2, len(inp)):
                sentences.append(inp[i])

        return sentences, queries

    def normalise(self, variables, num):
        variables = variables.split(",")
        for i in range(len(variables)):
            # Only normalise variables
            if is_variable(variables[i]):
                # Change variables to name + _ + sentence number
                variables[i] = variables[i] + "_" + str(num)
        return ",".join(variables)


    def parse(self, sentence, num):
        sentence = sentence.replace(" ", "")  # Replace spaces
        sentence = sentence.replace(")", "")  # Remove unrequired quotes
        sentence = sentence.split("|")
        sentence_tuple = []  # To store tuples [(Predicate, variables), .....]
        for t in sentence:
            p, v = t.split("(")
            v = self.normalise(v, num)
            sentence_tuple.append((p, v))
        return sentence_tuple


def main():
    parser = Parser("input.txt")
    sentences, queries = parser.read_input()
    kb = KnowledgeBase()
    i = 0
    for sen in sentences:
        t = parser.parse(sen, i)
        kb.tell(t)
        i += 1

    for query in queries:
        t = parser.parse(query, 0)
        result = kb.ask(t)
        if has_not(query):
            print not result
        else:
            print result


if __name__ == "__main__":
    main()